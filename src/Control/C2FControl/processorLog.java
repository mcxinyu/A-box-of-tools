...
//        File f = new File("/Users/huangyuefeng/Downloads/CDD/SZ01A.log");
        ProcessorLog rc = new ProcessorLog(f);
...
//        rc.processorSingleLog(f);
     ...
     */
    public ProcessorLog(File logFile) {
...
//                    System.out.println(lineContent);
                    ...
                    while (j<ss.length){
                        if (ss[j]!=null && ss[j].contains("CGI")){
                            sectors[0] = ss[j+1].substring(0,7);
                            sectors[1] = ss[j+1].substring(16,20);
                            sectors[2] = ss[j+1].substring(21,26).trim();
                            sectors[3] = ss[j+1].substring(30,32);
                            sectors[4] = ss[j+1].substring(36,43).trim();
                            sectors[5] = ss[j+4].substring(52,ss[j+4].length());
//                            System.out.println(sectors[0]+"-"+sectors[1]+"-"+sectors[2]+"-"+sectors[3]+"-"+sectors[4]+"-"+sectors[5]);
                            String line = "MSC\t"+bscName+"\tEricsson\t"+sectors[0].substring(0,6)+"\t22.68933\t113.77698\t"+sectors[0]+"\t"+sectors[1]+"\t"+sectors[2]+"\tNew\t50\t"+sectors[4]+"\t"+sectors[3]+"\tTRUE\tRXOTG\tTRUE\t20\tTRUE\t10\tRandom\tNo Preference";
                        }
                        j++;
                    }
                }
